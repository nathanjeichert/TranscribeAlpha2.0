  ...
  useEffect(() => {
    ...
    const syncToStart = () => {
      try {
        ...
        // Ignore failures prior to metadata loading
      }
    }
  ...
  const handleImport = useCallback(
    async (event: FormEvent<HTMLFormElement>) => {
      ...
      try {
        ...
        setImportXmlFile(null)
        setImportMediaFile(null)
        setCreationMessage('Transcript imported. You can now build clips from it.')
    ...
    [importMediaFile, importXmlFile, importing, onSessionRefresh],
  )
  ...
  const renderImportCard = () => (
    <div className="card">
      <div className="card-header">
        <h2 className="text-xl font-medium">Import existing transcript</h2>
      </div>
      <div className="card-body space-y-4">
        <p className="text-sm text-primary-700">
          Bring in an OnCue XML file and optional media to start clipping without running a new transcription.
        </p>
        ...
        <form className="space-y-4" onSubmit={handleImport}>
          ...
          <div>
            <label className="text-xs font-medium text-primary-700">Media file (optional)</label>
            <input
              type="file"
              accept="audio/*,video/*"
        ...
        <div className="card">
          ...
          <div className="card-body space-y-4">
            <p className="text-primary-700">
              Upload and process a transcript to enable clip creation. Once a transcript session is loaded, you can define
              precise clip ranges and export perfectly synced media and transcript files.
            </p>
            <p className="text-sm text-primary-600">
              Import an existing transcript below or generate a new one from the Transcribe tab.
            </p>
          </div>
        </div>
        {renderImportCard()}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                disabled={!clipBounds || !effectiveMediaUrl || isSubmitting || previewing}
              >
          ...
          <div className="card-body space-y-4">
            {effectiveMediaUrl ? (
              <div className="bg-primary-900 rounded-lg p-4">
                {isVideo ? (
                  <video ref={videoRef} key={effectiveMediaUrl} src={effectiveMediaUrl} controls className="w-full rounded" />
                ) : (
                  <audio ref={audioRef} key={effectiveMediaUrl} src={effectiveMediaUrl} controls className="w-full" />
                )}
              </div>
            ) : (
          ...
          <div className="card">
            ...
            <div className="card-body space-y-4">
              {clipHistory.length === 0 && (
                <p className="text-primary-700">No clips yet. Create your first clip to see it here.</p>
              )}
