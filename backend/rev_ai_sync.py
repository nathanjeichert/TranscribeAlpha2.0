...
class RevAIAligner:
    ...
    def submit_alignment_job(self, audio_url: str, transcript_url: str, metadata: str = "") -> str:
        """Submit alignment job to Rev AI using audio and transcript URLs."""
    ...
    def align_transcript(
        ...
        if not transcript_url:
            raise ValueError("transcript_url is required for Rev AI alignment")

        # Step 2: Submit job with media + transcript URLs
        try:
            # Submit job
            logger.info("Submitting alignment job with %d words", len(plain_text_words))
            job_id = self.submit_alignment_job(audio_url, transcript_url)
